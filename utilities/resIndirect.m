function [res,f] = resIndirect(X,n,m,odeOpts)
...
idx_q  = 2+2*n;
idx_u0 = 2+2*n+(1:m);
idx_lambda = 2+2*n+m+(1:2);
idx_gamma  = 2+2*n+m+2+1;
idx_vAVG   = 2+2*n+m+2+2;
...
[xT,yT,pT,uT] = flowIndirect(T,x0,p0,q,u0,gamma,odeOpts);
